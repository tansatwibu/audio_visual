...
class AudioVisualPose7layerUNet(nn.Module):
    def __init__(self, ngf=64, input_nc=2, output_nc=2):
        super(AudioVisualPose7layerUNet, self).__init__()
        #initialize layers
        self.audionet_convlayer1 = unet_conv(input_nc, ngf)
        self.audionet_convlayer2 = unet_conv(ngf, ngf * 2)
        self.audionet_convlayer3 = unet_conv(ngf * 2, ngf * 4)
        ...
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.fc1 = nn.Linear(128 * 17 * 10, 512)
    ...
    def forward(self, x, visual_feature, pose_feat):
        ...
        visual_first_size = visual_feature.size(0)
        visual_feature = visual_feature.view(visual_first_size, 1, 1, 256).repeat(1,68,20,1)
        pose_feat = pose_feat.view(visual_first_size, 68, 20, 256)

        x = torch.cat((pose_feat, visual_feature), dim=3)
        # print("x size: ", x.size())
        x = x.view(visual_first_size, 512, 20, 68)
        x = self.conv1(x)
        x = nn.ReLU()(x)
        x = self.conv2(x)
        x = nn.ReLU()(x)
        x = self.pool(x)
        x = x.view(x.size(0), -1)
        # print('x before fc1 size: ', x.size())
        x = self.fc1(x)
        x = nn.ReLU()(x)
...
class AudioPose7layerUNet(nn.Module):
    def __init__(self, ngf=64, input_nc=2, output_nc=2):
        ...
        # self.conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)
        # self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        # self.fc1 = nn.Linear(128 * 128 * 34 * 2, 512)
    ...
    def forward(self, x,visual_feature, pose_feat):
        ...
        visual_first_size = visual_feature.size(0)
        # visual_feature = visual_feature.view(visual_first_size, 1, 1, 256).repeat(1,68,20,1)
        pose_feat = pose_feat.view(visual_first_size, 256, 20, 68)
        x = pose_feat
        # print(x.shape)
        # x = torch.cat((pose_feat, visual_feature), dim=3)
        # x = x.view(visual_first_size, 512, 20, 68)
...
class AudioVisual7layerUNet(nn.Module):
    def __init__(self, ngf=64, input_nc=2, output_nc=2, visual_dim=512):
        ...
        self.audionet_upconvlayer1 = unet_upconv(ngf * 8 + visual_dim, ngf * 8)
        ...
        self.audionet_upconvlayer5 = unet_upconv(ngf * 8, ngf * 2)
        self.audionet_upconvlayer6 = unet_upconv(ngf * 4, ngf)
        self.audionet_upconvlayer7 = unet_upconv(ngf * 2, output_nc, outermost=True) 
    ...
    def forward(self, x, visual_feat):
        ...
        # 2. Visual Feature Bottleneck
        if len(visual_feat.shape) == 2:
            visual_feat = visual_feat.view(visual_feat.size(0), visual_feat.size(1), 1, 1)
        elif len(visual_feat.shape) == 3: 
