...
def augment_image(image):
	...
	image = enhancer.enhance(random.random()*0.6 + 0.7)
	return image
# npy_path = "/ext_data2/students/hainghiem/faster-rcnn.pytorch-pytorch-1.0/detection_results/duet/acoustic_guitar violin/G-25ZOVbGTE_2.npy"
...
def get_clip_name(npy_path):
    ...
    #return os.path.basename(npy_path)

def get_clip_class(npy_path):
    part = npy_path.split("/")
    return part[6]

def get_frame_root(npy_path):
    frame_path = npy_path[:-4].replace("/ext_data2/students/hainghiem/faster-rcnn.pytorch-pytorch-1.0/detection_results", "/ext_data2/manhnh/MUSIC_dataset/output_img")
    return frame_path
...
def get_audio_root(npy_path):
    audio_path = []
    if "duet" in npy_path:
        audio_path.append("/ext_data2/manhnh/MUSIC_dataset/reshape_11025/duet/top_detections_two_classes/")
    elif "solo" in npy_path:
        audio_path.append("/ext_data2/manhnh/MUSIC_dataset/reshape_11025/MUSIC_solo/top_detections_two_classes/")
    return audio_path[0]
...
class AudioVisualMUSICDataset(BaseDataset):
    def initialize(self, opt):
        ...
        for detection in detections:
            ...
            # detection = /ext_data2/manhnh/MUSIC_dataset/output_img/MUSIC_solo2/acoustic_guitar/XTKxjzg1xwM_1
            vidname = get_vid_name(detection) #get video id
            if "duet" in detection:
                npy_dectect = "/ext_data2/students/hainghiem/faster-rcnn.pytorch-pytorch-1.0/detection_results/duet/" + get_clip_class(detection) + "/" + get_clip_name(detection) + ".npy"
            elif "MUSIC_solo2" in detection:
                npy_detect = "/ext_data2/students/hainghiem/faster-rcnn.pytorch-pytorch-1.0/detection_results/MUSIC_solo2/" + get_clip_class(detection) + "/" + get_clip_name(detection) + ".npy"
    ...
    def __getitem__(self, index):
        ...
        for n in range(self.NUM_PER_MIX):
            ...
            for i in range(detection_bbs.shape[0]):
                ...
                frame_path = os.path.join(get_frame_root(clip_det_paths[n]), str(int(detection_bbs[i,0])).zfill(6) + '.png')
